...
function ms_servers() {
    while true; do
        ...
        case $choice in
            1)
                run_monitoring "111" "monitor_master1"
                echo "已在后台启动 Cluster_1 监控脚本 (会话名: 111)"
                ;;
            2)
                run_monitoring "222" "monitor_master2"
            ...
            4)
                ...
                (crontab -l; echo "0 6 * * * screen -X -S Cluster_1Master quit && screen -X -S Cluster_2Master quit") | crontab -
                echo "已设置服务器维护任务，每天早上6点执行！"
                ;;
            5)
                # 删除 crontab 中的服务器维护任务
                (crontab -l | grep -v "screen -X -S Cluster_1Master quit" | grep -v "screen -X -S Cluster_2Master quit") | crontab -
