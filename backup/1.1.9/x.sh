...
function ms_servers() {
    while true; do
        echo "============================================"
        echo "请选择要执行的操作:"
        echo "1. 监控 Cluster_1 崩溃重启"
        echo "2. 监控 Cluster_2 崩溃重启"
        echo "3. 关闭监控脚本"
        echo "4. 设置服务器维护任务（每天6点重启）"
        echo "5. 关闭服务器维护任务"
        echo "0. 返回主菜单"
        ...
        case $choice in
            1)
                # 在 screen 会话中定义函数并执行
                screen -dmS "monitor_Cluster_1" bash -c '
                    monitor_master1() {
                        if screen -list | grep -q "Cluster_1Master"; then
                            echo "Cluster_1Master正常运行中"
                        else
                ...
                '
                echo "✅ Cluster_1 监控已在后台启动（会话名：monitor_Cluster_1）"
                ;;
            2)
                # 在 screen 会话中定义函数并执行
                screen -dmS "monitor_Cluster_2" bash -c '
                    monitor_master2() {
                        if screen -list | grep -q "Cluster_2Master"; then
                            echo "Cluster_2Master正常运行中"
                        else
                ...
                '
                echo "✅ Cluster_2 监控已在后台启动（会话名：monitor_Cluster_2）"
                ;;
            3)
                # 关闭所有监控脚本
                screen -X -S monitor_Cluster_1 quit 2>/dev/null
                screen -X -S monitor_Cluster_2 quit 2>/dev/null
                echo "✅ 已关闭所有监控脚本"
                ;;
            4)
                # 设置每天6点自动关闭服务器
                (crontab -l 2>/dev/null; echo "0 6 * * * screen -X -S Cluster_1Master quit; screen -X -S Cluster_2Master quit") | crontab -
                echo "✅ 已设置每天 6:00 自动关闭服务器"
                ;;
            5)
                # 删除定时任务
                (crontab -l 2>/dev/null | grep -v "Cluster_1Master quit" | grep -v "Cluster_2Master quit") | crontab -
                echo "✅ 已删除服务器定时维护任务"
                ;;
            0)
                echo "返回主菜单..."
