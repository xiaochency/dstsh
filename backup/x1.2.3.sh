...
Install_dst() {
    ...
    mkdir ~/steamcmd
    cd ~/steamcmd
    # 检查 steamcmd_linux.tar.gz 是否存在
    file_name="steamcmd_linux.tar.gz"
    check_for_file "$file_name"

    if [ $? -eq 0 ]; then
        echo "$file_name 存在，正在删除..."
        rm "$file_name"  # 删除文件
    else
        echo "$file_name 不存在，继续下载steamcmd"
    fi
...
function ms_servers() {
    while true; do
        echo "============================================"
        echo -e "\e[32m请选择要执行的操作:\e[0m"
        echo -e "\e[32m1. 监控Cluster_1崩溃重启\e[0m"
        echo -e "\e[32m2. 监控Cluster_2崩溃重启\e[0m"
        echo -e "\e[32m3. 关闭监控脚本\e[0m"
        echo -e "\e[32m4. 设置服务器维护任务\e[0m"
        echo -e "\e[32m5. 关闭服务器维护任务\e[0m"
        ...
        case $choice in
            1)
                ...
                echo -e "\e[32m已在后台启动 Cluster_1 监控脚本 (会话名: 111)\e[0m"
                ;;
            2)
                run_monitoring "222" "monitor_master2" "monitor_caves2"
                echo -e "\e[32m已在后台启动 Cluster_2 监控脚本 (会话名: 222)\e[0m"
                ;;
            3)
                screen -list | grep -E '111|222' | cut -d. -f1 | awk '{print $1}' | xargs kill
                echo -e "\e[32m已关闭监控脚本...\e[0m"
                ;;
            4)
                ...
                echo -e "\e[32m已设置服务器维护任务，每天早上6点执行！\e[0m"
                echo -e "\e[32m已设置维护通知任务，每天早上5点55分发送！\e[0m"
                echo -e "\e[32m已设置保存命令任务，每天早上5点58分发送！\e[0m"

                ;;
            5)
                # 删除 crontab 中的服务器维护任务
                crontab -r
                echo -e "\e[32m已删除所有服务器维护任务\e[0m"
                ;;
            0)
...
while true; do
    echo "-------------------------------------------------"
    echo "饥荒云服务器管理脚本1.2.3 By:xiaochency            "
    echo "-------------------------------------------------"
    echo "请选择一个选项:                                   "
    echo "-------------------------------------------------"
